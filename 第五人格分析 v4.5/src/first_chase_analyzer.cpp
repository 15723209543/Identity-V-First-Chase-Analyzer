    ...
    const std::vector<survivor>& survivors) const
{
    ...
    result.selected_auxiliary_trait = trait;
    ...
    {
        score_breakdown score = calculator_.calculate_base_score(map, hunter_role, trait, hunter_position, survivor_role, survivors);
    ...
    for (size_t i = 0; i < result.scores.size(); ++i)
    {
        ...
        calculator_.fill_probability_text(map, hunter_role, trait, result.survivors[i], result.scores[i]);
    }
