...
int first_chase_app::run()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            hunter hunter_obj;
            auxiliary_trait trait_obj;
            bool hunter_and_trait_selected = false;
            while (!hunter_and_trait_selected)
            {
                ...
                hunter_obj = hunter(*hunter_data);
                markers.back().display_name = hunter_data->name;

                const number_input_result trait_input = ui.ask_auxiliary_trait(game_map_obj, markers);
                if (trait_input.action == input_action::back)
                {
                    markers.back().display_name.clear();
                    continue;
                }

                const auxiliary_trait_static_data* trait_data = find_auxiliary_trait_data(trait_input.value);
                if (!trait_data)
                {
                    MessageBoxA(nullptr, "辅助技能编号无效，程序结束。", "错误", MB_OK);
                    return 1;
                }
                trait_obj = auxiliary_trait(*trait_data);
                hunter_and_trait_selected = true;
            }

            if (!hunter_and_trait_selected)
            {
                continue;
            }
            ...
            const std::string output = print::build_output_text(result);
            const std::string result_file_name = print::save_timestamped_result(output);
