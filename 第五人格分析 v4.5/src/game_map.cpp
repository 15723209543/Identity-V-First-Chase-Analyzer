...
const map_region_data* game_map::find_region_at(int x, int y) const
{
    ...
    // 永眠镇当前底图被用户顺时针旋转了 90 度。
    // 地图显示和寻路不改，只把点击坐标反算回 map_data.h 的原始排位方位后再识别区域。
    if (data_.id == 4)
    {
        region_x = clamp_int(y, 0, 1000);
        region_y = clamp_int(1000 - x, 0, 1000);
    }

    // 区域识别以 map_data.h 中每张图单独配置的排位方位为准。
    // 用户可能替换过地图朝向，不能用统一 3×3/4×3 粗切分强行套所有地图。
    for (const auto& region : data_.regions)
    {
        if (region_x >= region.x
            && region_x <= region.x + region.width
            && region_y >= region.y
            && region_y <= region.y + region.height)
        {
            return &region;
        }
    }
    ...
    for (const auto& region : data_.regions)
    {
        ...
        const double distance = (center_x - region_x) * (center_x - region_x) + (center_y - region_y) * (center_y - region_y);
...
map_resource_detail game_map::nearby_resource_detail(const map_position& position) const
{
    const map_geometry_static_data* geometry_data = geometry();
    map_resource_detail detail{ 0, 0, 0.0, 0.0, 0.0 };
    ...
    for (const auto& window : windows)
    {
        ...
        if (distance <= 80.0)
        {
            // 每一扇可用窗都给一个基础牵制系数，距离越近再给少量加成。
            // 窗通常更适合拉开身位，所以单个窗的系数略高于木板。
            detail.window_count += 1;
            detail.window_score += params.window_base + (80.0 - distance) / 80.0 * params.window_near_bonus;
        }
    }
    ...
    for (const auto& pallet : pallets)
    {
        ...
        if (distance <= 90.0)
        {
            // 每一块可用板都给一个基础牵制系数，距离越近说明越容易直接使用。
            detail.pallet_count += 1;
            detail.pallet_score += params.pallet_base + (90.0 - distance) / 90.0 * params.pallet_near_bonus;
        }
    }
    detail.total_score = std::min(detail.window_score + detail.pallet_score, params.resource_cap);
    return detail;
}

double game_map::nearby_resource_score(const map_position& position) const
{
    return nearby_resource_detail(position).total_score;
}
